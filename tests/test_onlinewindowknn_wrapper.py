...
from experiments.baselines import onlinewindowknn


class OnlineWindowKNNTest(unittest.TestCase):
    def test_fifo_update_evicts_oldest_feature(self):
        memory: list[list[float]] = []

        onlinewindowknn.fifo_update(memory, [1.0], max_size=2)
        onlinewindowknn.fifo_update(memory, [2.0], max_size=2)
        onlinewindowknn.fifo_update(memory, [3.0], max_size=2)
    ...
    def test_score_uses_memory_before_update(self):
        ...
        score = onlinewindowknn.score_against_memory([3.0], memory, metric="l2")
        onlinewindowknn.fifo_update(memory, [3.0], max_size=3)
    ...
    def test_stream_order_changes_scores_and_is_deterministic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def run_scores(stream: Path, output: Path) -> list[float]:
                onlinewindowknn.run(str(stream), str(dataset_root), str(output), config)
