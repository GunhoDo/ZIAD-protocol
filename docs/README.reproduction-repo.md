...
**This repository is an experiment-reproduction artifact.** The primary path
below regenerates the paper's numbers, tables, and figure by running the code.
Building the paper PDF from the already-committed table/figure files is a
secondary convenience, documented last.
...
| `docs/` | [`reproduction.md`](docs/reproduction.md) (repro map), [`development_pipeline.md`](docs/development_pipeline.md) (historical dev log), runtime/env notes |
...
## Building the paper PDF (optional convenience)

The 6 tables/figure the paper `\input`s are already committed, so the PDF builds
...
This refreshes the tracked tables (no-op if unchanged) and produces
`paper/paper.pdf`. This is a convenience for reading the manuscript — the
reproduction path above is the point of this repository.

---

## Historical development pipeline

The original end-to-end development log — smoke runs, the staged P0 tiers,
mini-matrices, full-category sweeps, and the `paper_allowed` review gates — is
preserved in [`docs/development_pipeline.md`](docs/development_pipeline.md). It
predates the final paper-candidate runs; treat its status lines as historical.
