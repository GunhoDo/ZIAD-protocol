# Development Pipeline & Status Log (historical)

> **Note.** This document is the original development log: the full smoke / P0 /
> mini-matrix / full-category-sweep pipeline used while building the protocol,
> together with the staged `paper_allowed=false` review gates. It is preserved
> for provenance and for anyone extending the harness. It is **not** the
> reproduction entry point.
>
> For reproducing the paper's experiments, start at the top-level
> [`README.md`](../README.md) and [`docs/reproduction.md`](reproduction.md).
> Many status lines below (e.g. `paper_allowed=false`, validation stream lengths
> `2`/`20`, "smoke evidence") describe intermediate development states that
> predate the final paper-candidate runs and do not reflect the committed paper.
