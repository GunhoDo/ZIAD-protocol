...
scores are min--max normalized per baseline before computing ECE. These
observations support the protocol analysis without asserting a universal
detector ranking.
...
A length-64 replication of OnlineWindowKNN yields $\Delta\text{B-I}{=}{+0.021}$
(95\% CI $[{+0.002},{+0.039}]$), confirming detectability at the main scale.
...
(iii)~\textbf{Diagnostic detectors.}  A length-64 replication confirms
OnlineWindowKNN is significant at both scales (length~64: $+0.021$,
CI $[{+0.002},{+0.039}]$; length~256: $+0.014$, CI $[{+0.005},{+0.022}]$).
OnlinePatchCoreLite~($K{=}8$) yields $+0.021$ at length~64 but
CI $[{-0.001},{+0.042}]$ includes zero; its length-256 result remains valid.
These probes validate the $\Delta$B-I estimator's computational path but do
not establish that real deployed methods produce detectable effects at either
scale.
