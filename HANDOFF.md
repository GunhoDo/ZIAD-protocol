...
- 제한:
  ...
  - 남은 memory-policy shards는 MVTec PatchCore Reservoir/Prototype-EMA이다.
...
- 제한:
  ...
  - 이후 MVTec PatchCore FIFO도 완료됐다. 남은 memory-policy shards는 MVTec PatchCore Reservoir/Prototype-EMA이다.
...
- P0 compact smoke summary: `experiments/summarize_p0_smoke.py` writes `results/latest/tables/p0_smoke_summary.csv`, `p0_smoke_summary_manifest.json`, and `p0_smoke_summary.tex`; output has 26 rows, memory policies `default/SCS`, `FIFO`, `Reservoir`, and `Prototype-EMA`, all `measured_smoke_summary`, `paper_allowed=false`
- P0 memory readiness cleanup: `results/latest/p0_shards/manifest.json` now reports status `p0_shard_plan_ready_memory_partial`, calibration shards `8/8` ready, ready memory-policy shards `10`, and 2 missing memory-policy shards
...
18. MVTec/VisA full-category calibration-axis smoke matrices는 8/8 ready이고, MVTec RareCLIP FIFO/Reservoir/Prototype-EMA, MVTec PatchCore FIFO, VisA RareCLIP FIFO/Reservoir/Prototype-EMA 및 VisA PatchCore FIFO/Reservoir/Prototype-EMA full-category memory-policy shards가 paper-ineligible measured smoke로 존재한다.
...
- full P0 matrix 미실행
- P0 shard manifest는 생성됐고 `temperature_scaling` smoke postprocessor 및 MVTec/VisA full-category calibration-axis smoke matrices는 구현됐지만, MVTec PatchCore Reservoir/Prototype-EMA full-category shards와 full reviewed P0 matrix는 아직 미실행
...
Calibration-axis smoke coverage는 8/8 ready다. 다음은 남은 memory-policy shards 중 하나를 `paper_allowed=false`로 실행하고, aggregate metrics/manifest/CRD-lite와 P0 compact summary를 갱신한다. 현재 남은 후보는 MVTec PatchCore Reservoir/Prototype-EMA이다.
