...
SOURCE_SWEEP_CONFIG="${SOURCE_SWEEP_CONFIG:-experiments/configs/visa_full_category_stream_matrix_anomalyclip.yaml}"
...
if [ ! -f "$SOURCE_SWEEP_CONFIG" ]; then
  echo "ERROR: AnomalyCLIP VisA source stream matrix config not found: $SOURCE_SWEEP_CONFIG" >&2
  exit 1
fi
